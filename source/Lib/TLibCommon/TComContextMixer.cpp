...
class Mixer {
	...
	newArray<short, 16> tx; // N inputs from add()  //!!!!!->!! N个神经网络节点作为input，相当于si
	newArray<short, 16> wx; // N*M weights  //权重数组
	newArray<int> cxt;  // S contexts  //
	int ncxt;        // number of contexts (0 to S)     //!!!!!->!!能够参考的上下文的个数
	int base;        // offset of next context        /// 
	int nx;          // Number of inputs in tx, 0 to N  //节点个数
	newArray<int> pr;   // last result (scaled 12 bits)   //!!!!!->!!最终我们要得到的联合概率
...
public:
	...
	{  ///选择cxt as one of 'range' neural networks to
		// use； range 后面取值为1
		assert(range >= 0);
		assert(ncxt < S);
		assert(cx >= 0);
	...
	int p()
	{
		...
		else {  // S=1 context   //只有一个context的情况
...
//////////////////////////// APM //////////////////////////////
//得到混合概率Pr之后，一般还要经过几个APM（Adaptive Probability Map），
//也就是二次信号估计（SSE），对概率做线性插值和修正，得到一个更加精确的预测结果
...
class APM {
	int index;     // last p, context  上一个p出现的索引
...
//////////////////////////// StateMap //////////////////////////
//用于将某个状态（或者上下文）映射得到模型的预测概率；
//该部分是对每一个上下文维持一个概率，初始值为0.5
//然后当某个上下文出现时，根据下一输入的bit y与预测概率之间的误差更新预测概率：
// p = p + η*(y - p); 其中η是learning rate
//η
...
Predictor::Predictor() : pr(2048)   //pr的初始值为2048
{
}
